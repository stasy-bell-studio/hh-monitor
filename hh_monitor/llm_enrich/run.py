...
from hh_monitor.llm_enrich.prompt import build_prompt, parse_response
...
async def _enrich_one(
    ...
    *,
    ...
    if cached is not None:
        ...
        raw_resp = await llm_client.chat_completion(prompt)
    ...
    score_total = round(0.3 * fit_score_val + 0.7 * llm_resp.llm_score)

    log_ctx.info(
        "llm_enrich.scored",
        fit_score=fit_score_val,
        llm_score=llm_resp.llm_score,
        score_total=score_total,
        verdict=llm_resp.llm_verdict,
    ...
    await session.execute(
        ...
        .values(
            fit_score=fit_score_val,
            llm_score=llm_resp.llm_score,
            llm_verdict=llm_resp.llm_verdict,
            llm_comment=llm_resp.llm_comment,
            llm_red_flags=llm_resp.llm_red_flags,
            llm_real_role=llm_resp.llm_real_role,
