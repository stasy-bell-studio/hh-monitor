...
Структура ответа: ровно JSON с 9 полями:
  ...
  "facts_confirmed" — что подтверждено фактами из резюме (даты, должности, цифры, география). Возвращать как строку или массив строк; НЕ как объект/словарь.
  "weak_spots" — слабые места и что не подтверждено (пробелы, размытые формулировки, отсутствие KPI/P&L/штата/цифр). Возвращать как строку или массив строк; НЕ как объект/словарь.
  "red_flags" — красные флаги и несостыковки (сроки <1.5 года, понижения, разрывы, скачки индустрий). Возвращать как строку или массив строк; НЕ как объект/словарь.
